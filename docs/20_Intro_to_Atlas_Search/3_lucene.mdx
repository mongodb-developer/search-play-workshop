...
What Lucene, and MongoDB Search, call an "index" is really a collection of separate individual
...
mapped as `embeddedDocuments` (a topic covered later). To differentiate the terminology, MongoDB Search 
...
MongoDB Search translates its search operators directly to Lucene's `Query` API.
