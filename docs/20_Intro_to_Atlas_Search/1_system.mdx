...
MongoDB Search provides powerful findability capabilities to your data collections.
...
Applications communicate to MongoDB Search through the same mechanism as
...
The MongoDB Search `mongot` process, built on Apache Lucene, interfaces with the `mongod`
...
The `mongot` process performs the following tasks:
  * Creates MongoDB Search indexes based on the index definition.
  * Monitors change streams for the current state of the documents.
  * Processes MongoDB Search queries and returns the document IDs and other search metadata for
...
The MongoDB Atlas platform makes MongoDB Search deployable in either a coupled architecture alongside
the database nodes, or on separate dedicated Search Nodes. Dedicated Search Nodes provide separation
of concerns, alleviating resource contention. Dedicated Search Nodes are recommended for production workloads.

## Atlas coupled MongoDB Search
![coupled nodes](/img/coupled.png)

## Dedicated MongoDB Search Nodes
