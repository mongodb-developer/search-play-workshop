...
MongoDB Search is quite type sensitive. The BSON type of each field is used to determine which index mapping type to use. And at query time, the search operator type of the `value` is used to determine which indexed type to query for a particular field/path name.
